...
## Methods: constrained optimization with numerical simulator
...
obs_loc = vec(obs_loc)
obj_well(z) = .5 * norm(obs_loc .* (S(T(Float64.(log.(box_K(G1(z)[:,:,1,1]) .* Float32(md)))), f)[1:length(tstep)*prod(n)]-state[1:length(tstep)*prod(n)]))^2f0
...
for j=1:niterations
    ...
    @time global fval, gs = Flux.withgradient(() -> obj_ad(z), Flux.params(z))
    g = gs[z]
    fhistory[j] = fval
    p = -g/norm(g, Inf)
    ...
    try
        global step, fval = ls(f_, 5f0 * step, fval, dot(g, p))
    catch e
        global τ = τ * β
        global step, fval = ls(f_, 5f0 * step, fval, dot(g, p))
    end
    ...
    save_dict = @strdict λ subsamp j snr z K0 logK0 step niterations nv nsrc nrec survey_indices fhistory β τinit τend
    @tagsave(
    ...
    ## save figure
    fig_name = @strdict λ subsamp j snr niterations nv nsrc nrec survey_indices β τinit τend

...
obs_loc = vec(obs_loc)
obj_well(z) = .5 * norm(obs_loc .* (vec(SFNO(G1(z)[:,:,1,1])) - state_true))^2f0
...
for j=1:niterations
    ...
    try
        global step, fval = ls(f_, 2f0 * step, fval, dot(g, p))
    catch e
        global τ = τ * β
        global step, fval = ls(f_, 2f0 * step, fval, dot(g, p))
    end
    ...
    save_dict = @strdict λ subsamp β τinit τend j snr K0_save step niterations nv nsrc nrec survey_indices fhistory fnoerror
    @tagsave(
    ...
    ## save figure
    fig_name = @strdict λ subsamp β τinit τend j snr niterations nv nsrc nrec survey_indices

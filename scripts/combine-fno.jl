...
obs_loc = vec(obs_loc)
obj_well(K0) = .5 * norm(obs_loc .* ((vec(SFNO(K0))|>cpu) - state_true))^2f0
...
for j=1:niterations
    ...
    save_dict = @strdict λ subsamp j snr K0_save step niterations nv nsrc nrec survey_indices fhistory fnoerror
    @tagsave(

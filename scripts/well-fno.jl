...
obs_loc = vec(obs_loc)
state_true = state_true |> cpu
obj(logK0) = .5 * norm(obs_loc .* ((vec(SFNO(exp.(logK0)/Float32(md)))|>cpu) - state_true))^2f0
...
for j=1:niterations
    ...
    # linesearch
    function f_(α)
        misfit = obj(Float32.(box_logK(logK0 .+ α * p)))
        @show α, misfit
        return misfit
    end
    ...
    global logK0 = Float32.(box_logK(logK0 .+ step .* p))
